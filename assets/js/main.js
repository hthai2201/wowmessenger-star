...
const DEFAULT_CONFIG = {
  question: "Ngôi sao có tọa độ [ILY4EV-99] tên là?",
  answers: ["Sao Thủy", "Sao Mộc", "Sao Kim", "Sao Băng"],
  imageUrl: null,
  message: '"Sao" anh yêu em nhiều đến thế ❤️',
};
...
async function loadImageBase64FromJson(e) {
  // Check if we're looking for demo_moon_sample and have an imageUrl
  if (e === "demo_moon_sample" && customConfigData.imageUrl) {
    try {
      console.log("Fetching image from URL:", customConfigData.imageUrl);
      const response = await fetch(customConfigData.imageUrl);
      if (!response.ok)
      ...
      // Fall back to JSON if URL fetch fails
    }
  }
...
// Read custom configuration data from URL parameters
function readCustomConfigFromURL() {
  try {
    ...
    if (encodedData) {
      ...
      console.log("Custom configuration loaded:", customConfigData);
      return customConfigData;
    }
  } catch (error) {
    console.error("Error reading custom configuration from URL:", error);
  }

  // Use defaults if no custom data
  customConfigData = { ...DEFAULT_CONFIG };
  return customConfigData;
}
...
function createQuestionUI() {
  ...
  (t.className = "question-text"),
    (t.textContent = customConfigData.question),
    e.appendChild(t);
  const o = document.createElement("div");
  o.className = "answer-options";
  customConfigData.answers.forEach((e, t) => {
...
function init() {
  // Read custom configuration from URL parameters
  readCustomConfigFromURL();
...
function createLoveText() {
  console.log("[text love] Bắt đầu tạo text", customConfigData.message);
  const e = customConfigData.message,
    t = window.innerWidth,
    o = window.innerHeight,
